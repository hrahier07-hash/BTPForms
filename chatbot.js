...
(function () {
  ...
  async function send(text) {
    ...
    // KB locale d'abord (réponse instantanée pour les cas connus)
    const localAnswer = findAnswer(q);
    if (localAnswer) {
      await botReply(localAnswer);
    } else {
      // Fallback vers Grok AI via /api/chat
      ...
      try {
        ...
        if (res.ok) {
          ...
          addMsg(`Je n'ai pas pu répondre à cette question. Contactez-nous via <a href="contact.html" style="color:#3282DE;text-decoration:underline">ce formulaire</a>.`, 'bot');
        }
      } catch (e) {
        removeTyping();
        addMsg(`Je rencontre un problème technique. Contactez-nous via <a href="contact.html" style="color:#3282DE;text-decoration:underline">ce formulaire</a>.`, 'bot');
      }
    }
